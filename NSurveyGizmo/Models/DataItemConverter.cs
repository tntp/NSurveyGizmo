    ...
    public class DataItemConverter : JsonConverter
    {
            ...
            while (reader.TokenType == JsonToken.PropertyName)
            {
                ...
                else if (property.PropertyType == typeof(List<SurveyQuestion>))
                {
                    ...
                    foreach (var questionObject in results.Values)
                    {
                        ...
                        {
                            var questionOptions = JsonConvert.DeserializeObject<Dictionary<string, object>>(questionJObject["options"].ToString());
                ...
                reader.Read(); // Skip the , or } if we are at the end
            }
            return value;
        }
        ...
        private static SurveyQuestion GetSurveyQuestion(JObject questionJObject)
        {
            var surveyQuestion = new SurveyQuestion
            {
                ...
                _type            = questionJObject.GetString("base_type"),
                _subtype         = questionJObject.GetString("type"),
                question         = questionJObject.GetString("question"),
                QuestionResponse = questionJObject.GetString("answer")
            };
